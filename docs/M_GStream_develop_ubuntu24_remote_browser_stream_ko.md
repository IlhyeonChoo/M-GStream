...
기존 `docs/M_GStream_modification_log_ko.md`가 저장소 전반의 수정 이력을 누적하는 문서라면,
...
- `docs/M_GStream_ubuntu24_remote_browser_stream_prep_review_ko.md`
...
- `cmake --build build-ninja-m2 --target M_GStreamViewer_app --parallel` 통과
- `cmake --install build-ninja-m2` 통과
- `env -u DISPLAY -u WAYLAND_DISPLAY ./install/bin/M_GStreamViewer_app --headless --render-width 64 --render-height 64 --snapshot /tmp/M_GStream_empty.png` 통과
  - 결과: `/tmp/M_GStream_empty.png`, `173` bytes, `64x64` PNG
- `env -u DISPLAY -u WAYLAND_DISPLAY ./install/bin/M_GStreamViewer_app --headless --render-width 640 --render-height 360 --path ../gaussian-splatting/eval/bonsai --snapshot /tmp/M_GStream_bonsai.png` 통과
  - 결과: `/tmp/M_GStream_bonsai.png`, `386837` bytes, `640x360` PNG
  - 로그: `GaussianView CUDA/GL interop enabled.`
- `timeout 2s env -u DISPLAY -u WAYLAND_DISPLAY ./install/bin/M_GStreamViewer_app --offscreen --nogui --width 64 --height 64` 실행 시 direct EGL 초기화 후 `EXIT:124` 확인
...
- build: pass
- installed executable: install/bin/M_GStreamViewer_app
- EGL backend used: direct EGL pbuffer (device/surfaceless/default fallback)
- interop: pass
- empty snapshot: /tmp/M_GStream_empty.png, 173 bytes, 64x64
- model snapshot: /tmp/M_GStream_bonsai.png, 386837 bytes, 640x360
...
- `renderer/server` 가 `M_GStream` shared library 에 `GLOB_RECURSE` 로 암묵 포함되던 상태를 끊고, `M_GStream_server` static target 으로 분리한다.
- `src/projects/M_GStream/CMakeLists.txt` 에 `SIBR_BUILD_REMOTE_STREAM` option 을 추가한다.
  - Linux 기본값: `ON`
  - Windows 기본값: `OFF`
...
- install / smoke
  ...
  - `./install/bin/M_GStreamViewer_app --help` 실행 통과
- source ownership
  - `build-ninja/build.ninja` 와 `compile_commands.json` 에서 `ServerProtocol.cpp`, `CameraPoseAdapter.cpp` 가 `M_GStream_server` 에만 매핑되는 것을 확인했다.
- static archive symbols
  - `ar -t build-ninja/src/projects/M_GStream/renderer/server/libM_GStream_server_rwdi.a` 결과에 `CameraPoseAdapter.cpp.o`, `ServerProtocol.cpp.o` 포함
...
- install artifact scope
  - `install/bin`, `install/lib` 에는 기존 `M_GStreamViewer_app`, `libM_GStream_rwdi.so` 만 설치되고, 별도 server runtime binary 나 `www` asset 은 추가되지 않았다.
...
- server target: M_GStream_server (STATIC)
...
- deferred scope:
  ...
  - no link from M_GStreamViewer_app to M_GStream_server
...
M4에서는 `M_GStreamViewer_app` 안에 `RemoteStreamServer` 를 연결해, no-display viewer 프로세스와 같은 수명주기로 동작하는 HTTP skeleton 을 추가했다.
...
./install/bin/M_GStreamViewer_app --help
...
- endpoint smoke
  - `/healthz`: `200 OK`, JSON body 에 `ok=true`, `service=M_GStream_remote_stream`, `version=m4-http-skeleton`, `renderer.initialized=true`, `renderer.has_manifest=false`, `renderer.frame_index` / `renderer.app_time_sec` 포함 확인
  - `/`: `200 OK`, `text/html`
...
- static assets: pass (install/resources/M_GStream/server/www)
- /healthz: pass
...
```bash
cmake --build build-ninja --target M_GStreamViewer_app --parallel
LD_LIBRARY_PATH="$(find build-ninja -type f -name '*.so*' | sed 's#/[^/]*$##' | sort -u | paste -sd:):install/bin:install/lib" ./build-ninja/src/projects/M_GStream/apps/M_GStreamViewer/M_GStreamViewer_app   --headless --server   --listen-host 127.0.0.1   --listen-port 18080   --render-width 640 --render-height 360   --stream-width 640 --stream-height 360 --stream-fps 5   --path ../gaussian-splatting/eval/bonsai
...
- `cmake --build build-ninja --target install --parallel` 통과
  ...
  - app만 install할 때는 `cmake --build build-ninja --target M_GStreamViewer_app_install --parallel` 을 사용한다.
...
- installed app smoke:
  - `./install/bin/M_GStreamViewer_app --help` 통과
...
cmake --build build-ninja --target M_GStreamViewer_app --parallel
cmake --build build-ninja --target install --parallel
./install/bin/M_GStreamViewer_app   --headless --server   --listen-host 127.0.0.1   --listen-port 18080   --render-width 640 --render-height 360   --stream-width 320 --stream-height 180   --stream-fps 2   --path ../gaussian-splatting/eval/bonsai
...
- 문서
  ...
  - `README.md`, `docs/M_GStream_ubuntu24_toolchain_status_ko.md` 업데이트
...
- 설치 서버를 `--www-root src/projects/M_GStream/renderer/server/www` 로 실행한 뒤 `/`, `/app.js`, `/styles.css` 정적 자산 서빙 확인: PASS
- source 자산 기준 smoke
