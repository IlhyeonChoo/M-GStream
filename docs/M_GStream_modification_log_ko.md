...
- `src/projects/M_GStream/renderer/CMakeLists.txt`
  - 프로젝트 전용 cache 변수 `MGSTREAM_CUDA_ARCHITECTURES`를 추가했다.
  - 기본값을 `86-real;89-real;90-real;120`으로 두고, `M_GStream` 본체와 `CudaRasterizer` 둘 다 이 값을 명시적으로 사용하도록 고정했다.
...
- `docs/M_GStream_windows_portable_bundle_ko.md`
  - 새 Windows PC direct build 예시에 `-DMGSTREAM_CUDA_ARCHITECTURES=86-real;89-real;90-real;120`를 명시했다.
...
- 관측 로그 위치
  - `src/projects/M_GStream/renderer/subsystem/rendering_system/GaussianView.cpp:81`
- 관측 함수
...
- `MGStreamViewer` / `RenderingSystem` / `GaussianView` 연결
...
- `plan/2026-04-08-ubuntu24-remote-browser-stream-prep.md`도 함께 작성했지만, 현재 저장소의 `.gitignore`에 `plan` 디렉터리가 포함되어 있어 기본 `git status`에는 나타나지 않는다. 따라서 작업 이력의 기준 문서는 본 `docs/M_GStream_modification_log_ko.md` 항목으로 본다.

### 0.9 remote browser stream 선행 작업 리뷰 반영

`docs/M_GStream_ubuntu24_remote_browser_stream_prep_review_ko.md`의 지적을 반영해,
...
`docs/M_GStream_develop_ubuntu24_remote_browser_stream_followup_plan_ko.md`에서
...
- `src/projects/M_GStream/renderer/server/examples/control_messages/valid_set_camera_pose_default.json`
- `src/projects/M_GStream/renderer/server/examples/control_messages/invalid_missing_fovy.json`
- `src/projects/M_GStream/renderer/server/examples/control_messages/invalid_position_length.json`
- `src/projects/M_GStream/renderer/server/examples/control_messages/invalid_fovy_out_of_range.json`
- `src/projects/M_GStream/renderer/server/examples/control_messages/invalid_forward_zero.json`
- `src/projects/M_GStream/renderer/server/examples/control_messages/invalid_forward_up_near_parallel.json`
- `src/projects/M_GStream/renderer/server/examples/control_messages/invalid_trailing_content.txt`
- `src/projects/M_GStream/renderer/server/www/README.md`
...
- 기존 hot path 파일은 수정하지 않는다.
- `main.cpp`, `MGStreamViewer`, `RenderingSystem`, `GaussianView`는 건드리지 않는다.
...
- 기본 인자 `tools/windows/build_windows_portable_bundle.ps1` 실행
  ...
  - bundle은 `M_GStreamViewer_app_rwdi.exe`를 선택했다.
- `tools/windows/build_windows_portable_bundle.ps1 -BuildRoot .\build-ninja -Config Debug` 실행
  ...
  - bundle은 `M_GStreamViewer_app_d.exe`를 선택했다.
- `tools/windows/check_windows_runtime.ps1`
...
- `docs/M_GStream_windows_portable_bundle_review_finding_response_ko.md`
...
- `tools/windows/package_windows_portable_bundle.ps1`
  ...
  - `MinSizeRel` config를 `M_GStreamViewer_app_msr.exe`로 올바르게 매핑했다.
  - executable fallback 후보 목록에도 `_msr`를 포함했다.
- `tools/windows/run_portable_bundle.cmd`
  - `selected_viewer_exe.txt`가 없을 때의 fallback 후보에 `M_GStreamViewer_app_msr.exe`를 추가했다.
- `docs/M_GStream_windows_portable_bundle_ko.md`
...
- Windows 로컬 환경에서 `M_GStream` 프로젝트가 **다시 빌드되고 실행되도록** 막고 있던 오류를 정리한다.
...
- `MGStreamViewer`의 asset 추가, 인스턴스 생성, 자산 교체 UI가 pointer 대신 asset 이름 기반으로 동작하도록 변경했다.
...
- `src/projects/M_GStream/renderer/subsystem/rendering_system/RenderGaussianInstance.hpp`
...
이를 해결하기 위해 `extlibs/CudaRasterizer/CudaRasterizer/CMakeLists.txt`와 `src/projects/M_GStream/renderer/CMakeLists.txt`를 다음과 같이 수정했다.
...
- `M_GStream` 본체 CUDA 타깃에도 같은 host compiler 옵션을 적용했다.
- `src/projects/M_GStream/renderer/CMakeLists.txt`의 `CMAKE_CUDA_ARCHITECTURES native` 설정을 그대로 활용하도록 맞췄다.
...
빌드 성공 이후에도 사용자 입장에서는 `build-ninja/.../M_GStreamViewer_app_rwdi.exe`를 직접 실행했을 때 다음 문제가 있었다.

- `sibr_system_rwdi.dll`, `sibr_graphics_rwdi.dll`, `M_GStream_rwdi.dll` 등 런타임 DLL을 찾지 못함
- 초기 `run_M_GStream_viewer.cmd` 스크립트가 프로젝트 루트를 한 단계 잘못 계산하여 잘못된 `install` 경로를 참조함

이를 해결하기 위해 다음 보완을 적용했다.

- `build-ninja/src/projects/M_GStream/apps/M_GStreamViewer/run_M_GStream_viewer.cmd`를 추가했다.
- wrapper는 실행 전 `PATH`에 필요한 build / extlibs 디렉터리를 넣고, `--appPath`를 현재 프로젝트의 `install` 경로로 전달하도록 구성했다.
- wrapper의 루트 경로 계산 버그를 수정해 `C:\\Users\\...\\M_GStream\\install`를 정확히 찾도록 고쳤다.
...
- `build-ninja/src/projects/M_GStream/apps/M_GStreamViewer/run_M_GStream_viewer.cmd`
...
- `build-ninja/src/projects/M_GStream/apps/M_GStreamViewer/M_GStreamViewer_app_rwdi.exe`
...
- 가장 안전한 실행 방법은 `build-ninja/src/projects/M_GStream/apps/M_GStreamViewer` 폴더에서 `run_M_GStream_viewer.cmd`를 실행하는 것이다.
- 현재 워크스페이스에서는 같은 폴더의 `M_GStreamViewer_app_rwdi.exe`를 직접 실행해도 동작하도록 필요한 DLL을 함께 배치해 두었다.
...
- `cmake --build build-ninja-m2 --target M_GStreamViewer_app --parallel` 통과
- `cmake --install build-ninja-m2` 통과
- `./install/bin/M_GStreamViewer_app --help` 실행 통과
...
- no-display empty snapshot smoke: `PASS`
  - `/tmp/M_GStream_empty.png`, `64x64`, `173` bytes
- no-display Gaussian snapshot smoke: `PASS`
  - `/tmp/M_GStream_bonsai.png`, `640x360`, `386837` bytes
...
- `src/projects/M_GStream/CMakeLists.txt`
  - `SIBR_BUILD_REMOTE_STREAM` option 추가
  - Linux 기본값 `ON`, Windows 기본값 `OFF`
- `src/projects/M_GStream/renderer/CMakeLists.txt`
  - `renderer/server` subtree 를 `M_GStream` 의 암묵 `GLOB_RECURSE` 소스 집합에서 제외
  - 조건부로 `renderer/server` 하위 디렉터리를 별도 target 으로 추가
- `src/projects/M_GStream/renderer/server/CMakeLists.txt`
  - `M_GStream_server` static target 추가
  - HTTP / WebSocket backend 를 `Boost.Beast` 로 고정
  - `TurboJPEG` 는 probe / summary 만 추가하고 hard requirement 는 M5 로 deferred
- `src/projects/M_GStream/renderer/server/Config.hpp`
...
cmake --build build-ninja --target M_GStream_server --parallel
cmake --build build-ninja --target M_GStream --parallel
...
- top-level project 에 `SIBR_BUILD_REMOTE_STREAM` option 을 추가했다.
  - Linux 기본값 `ON`
  - Windows 기본값 `OFF`
- `M_GStream` shared library 의 `GLOB_RECURSE` 결과에서 `renderer/server/*` 를 제거했다.
- `SIBR_BUILD_REMOTE_STREAM=ON` 일 때만 `renderer/server` 하위 디렉터리를 추가한다.
- `M_GStream_server` 정적 라이브러리를 새로 만들었다.
- `renderer/server/Config.hpp` 를 추가해 `SIBR_MGSTREAM_SERVER_EXPORT` macro 를 분리했다.
...
- configure summary 에서 다음 확인
  - `M_GStream remote stream modules: ON`
  - `M_GStream_server JSON backend: picojson`
  - `M_GStream_server HTTP/WebSocket backend: Boost.Beast`
  - `M_GStream_server JPEG backend: TurboJPEG not found...`
- `cmake --build build-ninja --target M_GStream_server --parallel` 통과
- `cmake --build build-ninja --target M_GStream --parallel` 통과
- `cmake --build build-ninja --target M_GStreamViewer_app --parallel` 통과
- `cmake --build build-ninja --target install --parallel` 통과
- `./install/bin/M_GStreamViewer_app --help` 실행 통과
- `build-ninja/build.ninja`, `compile_commands.json` 에서 `ServerProtocol.cpp`, `CameraPoseAdapter.cpp` 가 `M_GStream_server` 에만 속하는 것 확인
...
M4에서 `M_GStreamViewer_app` 와 `M_GStream_server` 를 실제 runtime 으로 연결하고, no-display viewer 위에서 동작하는 HTTP skeleton 을 구현했다.
...
- `src/projects/M_GStream/renderer/server/ServerProtocol.hpp`
- `src/projects/M_GStream/renderer/server/ServerProtocol.cpp`
- `src/projects/M_GStream/renderer/server/RemoteStreamServer.hpp`
- `src/projects/M_GStream/renderer/server/RemoteStreamServer.cpp`
...
- `MGStreamViewer` 에 health snapshot 용 getter 를 추가했다.
- `src/projects/M_GStream/CMakeLists.txt` 에서 `renderer` 를 `apps` 보다 먼저 add 하도록 순서를 바꿨다.
  - 이유: app CMake 가 `M_GStream_server` target 을 볼 수 있어야 app link 와 compile definition 이 적용된다.
- `apps/M_GStreamViewer/CMakeLists.txt` 에서 app 이 `M_GStream_server` 를 link 하고 `SIBR_MGSTREAM_REMOTE_STREAM_BUILD=1` compile definition 을 받도록 수정했다.
- `www` install destination 을 `resources/M_GStream/server/www` 로 맞췄고, runtime lookup 도 같은 install path 를 우선 탐색하게 했다.
...
- `cmake --build build-ninja --target M_GStreamViewer_app --parallel` 통과
- `cmake --build build-ninja --target install --parallel` 통과
- `./install/bin/M_GStreamViewer_app --help` 에 server 관련 옵션 노출 확인
- `./install/bin/M_GStreamViewer_app --offscreen --nogui --server --listen-host 127.0.0.1 --listen-port 18080 --width 640 --height 360` 기동 통과
  - startup log 에 `Initialization of direct headless EGL`
  - startup log 에 `RemoteStreamServer listening on 127.0.0.1:18080`
  - resolved `www root` 가 `install/resources/M_GStream/server/www` 로 잡힘
- `curl` smoke
  - `/healthz`: `200 OK`
  - `/`: `200 OK`
  - `/app.js`: `200 OK`
...
- `src/projects/M_GStream/renderer/server/RemoteStreamServer.hpp`
- `src/projects/M_GStream/renderer/server/RemoteStreamServer.cpp`
- `src/projects/M_GStream/renderer/server/JpegEncoder.hpp`
- `src/projects/M_GStream/renderer/server/JpegEncoder.cpp`
- `src/projects/M_GStream/renderer/server/MjpegStreamer.hpp`
- `src/projects/M_GStream/renderer/server/MjpegStreamer.cpp`
- `docs/M_GStream_develop_ubuntu24_remote_browser_stream_ko.md`
- `docs/M_GStream_remote_browser_stream_manual_checklist_ko.md`
- `src/projects/M_GStream/renderer/server/www/README.md`
...
- `install_runtime.cmake` 의 Linux `INSTALL_PDB` early-return 과 executable postfix 처리 버그를 수정해 `M_GStreamViewer_app_install` 이 runtime dependency 와 `www/` 를 실제로 설치하도록 정리했다.
...
- installed binary `./install/bin/M_GStreamViewer_app --help` 통과
...
- `MGStreamViewer` 에 `"Gaussian View"` camera getter / apply helper 를 추가했다.
...
- `node --check src/projects/M_GStream/renderer/server/www/app.js` 통과
...
## 16. 2026-04-15 manifest remote stream phase/status bridge

이번 변경은 기존 remote browser stream 에 manifest 관찰/제어 경로를 붙이는 데 집중했다. 핵심은 브라우저에서 phase 를 바꾸고, `/healthz` 와 `ready` 에서 manifest/streaming 상태를 바로 볼 수 있게 만드는 것이다.
...
- `src/projects/M_GStream/renderer/server/www/app.js`
- `src/projects/M_GStream/renderer/server/www/styles.css`
- `src/projects/M_GStream/renderer/server/examples/control_messages/valid_set_phase_default.json`

적용한 변경:

- `ServerProtocol`
  - `ControlMessageType::SetPhase` 와 `ControlMessage.phase` 를 추가했다.
  - `ParseControlMessageJson()` 에 `set_phase` 분기를 넣고 string field `phase` 를 파싱하도록 정리했다.
- `MGStreamViewer`
  - remote control 쪽에서 phase 를 바꾸고 phase 목록/manifest asset 수를 읽을 수 있게 `setCurrentPhase()`, `getAvailablePhases()`, `getManifestAssetCount()` 를 공개했다.
- `main.cpp`
  - `pumpRemoteControl()` 에 `SetPhase` case 를 추가했다.
  - renderer health snapshot 에 current phase / available phases / total assets / swap stats 를 실었다.
- `RemoteStreamServer`
  - `/healthz` 의 `renderer` 객체에 `current_phase`, `available_phases`, `total_assets`, `streaming` 하위 stats 를 추가했다.
  - WebSocket `ready` payload 에 manifest 상태 필드를 추가했다.
  - `ack` payload 에 `request_type` 을 넣어 `set_camera_pose` 와 `set_phase` 를 구분할 수 있게 했다.
- browser client (`www`)
  - `Manifest Status` 패널, phase dropdown/custom input, total asset 표시, streaming stats 필드를 추가했다.
  - `set_phase` 전송, `/healthz` 1 Hz polling, manifest panel 동기화 로직을 추가했다.
  - `set_phase` ack 만 status line 에 표시하고 camera pose ack churn 은 그대로 숨겼다.
- examples
  - `valid_set_phase_default.json` 을 추가해 WebSocket smoke 시 `set_phase` 회귀를 바로 찍을 수 있게 했다.

검증 결과:

- `cmake --build build-ninja --target M_GStreamViewer_app --parallel`: PASS
- `cmake --install build-ninja`: PASS
- `node --check src/projects/M_GStream/renderer/server/www/app.js`: PASS
- build-tree binary + `LD_LIBRARY_PATH` 로 manifest server runtime 검증: PASS
  - `/healthz`: `current_phase=alpha`, `available_phases=["alpha","beta"]`, `total_assets=1`, `streaming.required_gpu=1` 확인
  - `ws_control_smoke.mjs`: `ready.has_manifest=true`, `ack.request_type=set_phase` 확인
  - 후속 `/healthz`: `current_phase=default_phase` 로 반영 확인
- install binary 기준 manifest server runtime 재검증: PASS
  - 설치 트리 갱신 후 동일한 `ready` / `ack` / `/healthz` 확인
- 정적 client asset 확인
  - `/` 에 `Manifest Status`, `phase-select`, `toggle-health-poll` 존재 확인

메모:

- 검증용 manifest 는 로컬 절대 경로 모델을 가리키는 임시 파일로 생성/삭제했다. repo 에는 남기지 않았다.
- 설치 전에는 `install/bin` 이 M8 상태라 smoke 결과가 한 번 stale 하게 나왔고, 이후 `cmake --install build-ninja` 후 재검증해 해결했다.

- 상세 코드 비교: `docs/source_diff_details_44a8416_to_manifest_remote_stream/`


## 17. 2026-04-19 browser stream orientation + free-flight camera

이번 변경은 브라우저 remote stream 에서만 보이던 상하 반전과, 브라우저 카메라의 상하 회전 제한을 같이 정리했다.
핵심은 MJPEG 인코딩 직전의 행 방향 처리와 browser-side free-flight basis 재구성이다.
...
- `MjpegStreamer`
  - `RawFrame.bottom_up` 플래그를 실제 인코딩 경로에서 사용하도록 정리했다.
  - 현재 render target readback 은 display orientation 이 이미 맞춰진 상태로 취급하고, MJPEG 인코더에서 무조건 row reverse 하던 경로를 제거했다.
  - 결과적으로 브라우저 스트림은 renderer 최종 출력 방향을 그대로 유지한다.
- browser client (`app.js`)
  - `WORLD_UP` 기반 pitch clamp 를 제거했다.
  - yaw 는 current `up`, pitch 는 current `right` 기준으로 적용하고, 회전 후 매 프레임 직교 basis 를 다시 구성하도록 바꿨다.
  - `ArrowUp` / `ArrowDown` 과 마우스 Y drag 가 직상/직하를 지나도 계속 동작하도록 정리했다.
  - `Q` / `E` 는 기존대로 world Y 축 이동을 유지했다.
- browser assets
  - `index.html` key legend 를 free-look 기준으로 갱신했다.
  - `README.md` 에 local-basis free-flight 동작을 기록했다.

검증 결과:

- `node --check src/projects/M_GStream/renderer/server/www/app.js`: PASS
- `cmake --build build-ninja --target M_GStreamViewer_app --parallel`: PASS
- 런타임 시각 검증은 실제 브라우저/서버 조합에서 추가 확인이 필요하다.

메모:

- 이번 변경은 브라우저 stream / browser camera control 경로만 수정했다.
- `GaussianLoader`, scene transform, native viewer 좌표계는 건드리지 않았다.
