...
| `M_GStreamViewer_app` build | PASS |
| `install` target | PASS |
...
cmake --build build-ninja --target M_GStream -j4
cmake --build build-ninja --target M_GStreamViewer_app -j4
...
| check | result |
|---|---|
| `install/bin/M_GStreamViewer_app` | executable exists |
| `install/bin/libM_GStream_rwdi.so` | exists |
| `install/ibr_resources.ini` | exists |
| `install/shaders/core` | exists |
| `install/shaders/M_GStream` | exists |
| `ldd install/bin/M_GStreamViewer_app | rg 'not found'` | no matches |
...
| renderer public headers | `M_GStream` target의 renderer directory를 PUBLIC include path로 내보냈다. Public header들이 `"Config.hpp"`를 포함한다. |
...
```sh
timeout 8s install/bin/M_GStreamViewer_app --offscreen --nogui --width 320 --height 240 --vsync 0
```
...
```sh
xvfb-run -a timeout 8s install/bin/M_GStreamViewer_app --nogui --width 320 --height 240 --vsync 0
```
...
```sh
cd /home/ilhyeonchu/ReCompose3D/3DGS/M_GStream-desktop-server
install/bin/M_GStreamViewer_app --width 1280 --height 720 --vsync 0
```
