...
이 문서는 `M_GStream`를 **다른 Windows 10/11 PC에서도 비교적 바로 실행**하기 위한 기준 절차를 정리한다.
...
권장 실행 파일도 `M_GStreamViewer_app_rwdi.exe` 기준이다.
...
```powershell
cmd.exe /d /s /c '"C:\Program Files\Microsoft Visual Studio\2022\Community\Common7\Tools\VsDevCmd.bat" -arch=x64 && cmake --build build-portable --target M_GStreamViewer_app'
```

```powershell
cmd.exe /d /s /c '"C:\Program Files\Microsoft Visual Studio\2022\Community\Common7\Tools\VsDevCmd.bat" -arch=x64 && cmake --build build-portable --target M_GStreamViewer_app_install'
```

`cmake --install build-portable` 로 전체 설치를 시도하면, 현재 viewer에 직접 필요하지 않은 다른 타깃이 아직 안 빌드된 경우 실패할 수 있다.
Windows에서는 `M_GStreamViewer_app_install` 타깃을 표준 설치 경로로 사용하는 것을 권장한다.
...
지원 GPU 범위는 NVIDIA Turing(SM 7.5, RTX 20 시리즈 / T4 / Quadro RTX) 이상이다. Volta(SM 7.0), Pascal, Maxwell, Kepler 는 portable bundle 지원 대상이 아니다.
...
```powershell
.\install\scripts\M_GStream\run_installed_viewer.cmd
```

```powershell
& ".\install\bin\M_GStreamViewer_app_rwdi.exe" --appPath ".\install"
```
...
- `M_GStreamViewer_app` 빌드
- 빌드 직후 build tree의 CUDA 아키텍처가 `75-real;86-real;89-real;120` 범위를 포함하는지 검증
- `M_GStreamViewer_app_install` 실행
...
- `build\windows-portable-bundle\M_GStream-windows-portable.zip`
...
- `build\windows-portable-bundle\M_GStream-windows-portable`
...
- `Debug` -> `M_GStreamViewer_app_d.exe`
- `RelWithDebInfo` -> `M_GStreamViewer_app_rwdi.exe`
- `Release` -> `M_GStreamViewer_app.exe`
- `MinSizeRel` -> `M_GStreamViewer_app_msr.exe`
...
즉 runtime DLL 포함 책임은 package 단계가 아니라 `M_GStreamViewer_app_install` 단계에 있다.
...
- `install/scripts/M_GStream/run_installed_viewer.cmd`
- `install/scripts/M_GStream/check_windows_runtime.ps1`
- `install/docs/M_GStream_windows_portable_bundle_ko.md`
...
```cmd
run_M_GStream_viewer.cmd --manifest ".\manifests\mc_small_aerial_c36_neighbors_3x3.json"
```
