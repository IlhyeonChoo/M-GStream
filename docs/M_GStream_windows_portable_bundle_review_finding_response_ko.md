...
- `package_windows_portable_bundle.ps1`의 `Get-ExecutableCandidates`는 `MinSizeRel`을 `M_GStreamViewer_app.exe`로 매핑한다.
- 하지만 top-level `CMakeLists.txt`는 `CMAKE_MINSIZEREL_POSTFIX`를 `_msr`로 정의한다.
- 따라서 실제 `MinSizeRel` viewer 이름은 `M_GStreamViewer_app_msr.exe`가 되어야 한다.
...
- `package_windows_portable_bundle.ps1`
  - `MinSizeRel`을 `M_GStreamViewer_app_msr.exe`로 매핑한다.
...
5. `docs/M_GStream_windows_portable_bundle_ko.md`에 package-time preflight와 `MinSizeRel -> _msr` 규칙을 반영한다.
6. `docs/M_GStream_modification_log_ko.md`에 이번 후속 수정을 기록한다.
