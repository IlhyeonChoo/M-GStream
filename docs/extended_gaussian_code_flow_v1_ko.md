# Extended Gaussian 코드 실행 흐름 (Version 1)

작성일: 2026-04-08  
기준 브랜치: `develop/phase1-manifest-swap`

> 이 문서는 `docs/extended_gaussian_code_flow_phase0_ko.md` 이후의 **현재 기준 실행 흐름 스냅샷**이다.  
> 앞으로 흐름이 다시 바뀌면 같은 형식으로 `v2`, `v3` 문서를 추가하고, 기존 버전 문서는 과거 기록으로 유지한다.

## 관련 문서

- `docs/extended_gaussian_code_flow_phase0_ko.md`
...
- `ExtendedGaussianViewer::onRender()` 안에서 `ViewerContext`를 만들고, `RenderingSystem::tickStreaming()`이 매 프레임 실행된다.
...
`ExtendedGaussianViewer` 생성자에서 `--manifest`가 있으면 바로 `loadManifestFile(...)`가 호출된다.
...
현재 프레임 흐름의 실질적인 분기점은 `ExtendedGaussianViewer::onRender()`다.
...
1. `ExtendedGaussianViewer::loadManifestFile(...)`
2. `ManifestStore::load(...)`
3. `ResourceManager::registerManifest(...)`

### 카메라 위치에 따라 로딩 셋이 이상한 경우

1. `ExtendedGaussianViewer::focusCameraOnManifest()`
2. `ExtendedGaussianViewer::onRender()`
...
- 파일명 패턴:
  - `docs/extended_gaussian_code_flow_v2_ko.md`
  - `docs/extended_gaussian_code_flow_v3_ko.md`
- 각 문서 상단에 반드시 기록:
  - 작성일
  - 기준 브랜치
  - 이전 버전 대비 핵심 변화 3~5개
