...
이 문서는 Ubuntu 24.04 loopback 환경에서 `extended_gaussian` remote browser stream 기능의 M7 통합 검증 결과를 한 곳에 고정한다.
...
| app binary | `install/bin/extended_gaussianViewer_app` |
...
```sh
./install/bin/extended_gaussianViewer_app   --headless   --server   --listen-host 127.0.0.1   --listen-port 18180   --render-width 1280   --render-height 720   --stream-width 1280   --stream-height 720   --stream-fps 15   --path ../gaussian-splatting/eval/bonsai
```
...
| area | status | note |
|---|---|---|
| build / install | RUN_PASS | `extended_gaussianViewer_app` build 및 `install` target 통과 |
...
|---|---|---|---|---|
| V01 | build / install | RUN_PASS | `cmake --build build-ninja --target extended_gaussianViewer_app --parallel` 통과, `cmake --build build-ninja --target install --parallel` 통과 | local shell run, branch log / mod log append |
...
| V06 | single-client MJPEG | RUN_PASS | 15.03초, 205 frames, 13.64 FPS, first frame 115.8ms | `/tmp/extended_gaussian_m7/single_stream_summary.json` |
| V07 | two-client MJPEG | RUN_PASS | client A/B 각 166 frames, `active_clients_min=max=2` | `/tmp/extended_gaussian_m7/two_client_a_summary.json`, `two_client_b_summary.json`, `two_client_healthz.jsonl` |
| V08 | GET `/control` upgrade 안내 | RUN_PASS | `426 Upgrade Required`, JSON error body 확인 | local `curl -i` |
| V09 | WS valid payload apply | RUN_PASS | valid payload `ack sequence=2`, `/healthz`에서 `messages_applied=2`, pose 변경 확인 | `/tmp/extended_gaussian_m7/ws_smoke_summary.json` |
| V10 | WS invalid payload reject | RUN_PASS | `Missing required numeric field 'fovy'.` error, rejected counter 증가 | `/tmp/extended_gaussian_m7/ws_smoke_summary.json` |
| V11 | control flood / latest-only loop | RUN_PASS | 10초 orbit, 50 payload / 50 ack, `messages_applied=51`, `receive_to_apply.p95=7.575ms` | `/tmp/extended_gaussian_m7/ws_orbit_summary.json` |
| V12 | control-to-encoded proxy | RUN_PASS | `ack sequence=52` 이후 첫 `control_sequence=52` frame의 `ack_to_encoded_ms=69` | `/tmp/extended_gaussian_m7/control_visible_ws.json`, `control_visible_frames.jsonl` |
| V13 | 120초 local soak | RUN_PASS | 120.05초, 1690 frames, 14.08 FPS, health sample 24회 / failure 0회, RSS drift +28 KB (0.005%) | `/tmp/extended_gaussian_m7/soak_*` |
