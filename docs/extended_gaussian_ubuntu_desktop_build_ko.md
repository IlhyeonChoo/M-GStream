...
대상: Ubuntu desktop에서 Ninja로 `extended_gaussianViewer_app`을 빌드하고 실행하려는 작업자.
...
```sh
cd /home/ilhyeonchu/Documents/GitHub/extended_gaussian
...
```sh
cmake --build build-ninja --target extended_gaussianViewer_app -- -j1
```
...
```sh
ls -la install/bin/extended_gaussianViewer_app
ldd install/bin/extended_gaussianViewer_app | grep 'not found'
```
...
cmake --build build-ninja --target extended_gaussianViewer_app -- -j1
cmake --build build-ninja --target install
ldd install/bin/extended_gaussianViewer_app
./install/bin/extended_gaussianViewer_app
./install/bin/extended_gaussianViewer_app \
  --manifest manifests/mc_small_aerial_c36_neighbors_3x3_cpu36_gpu9.json
```
...
| 항목 | 결과 |
|---|---|
| `extended_gaussianViewer_app` target | PASS |
| Ninja `install` target | PASS |
| installed executable | `install/bin/extended_gaussianViewer_app` 존재 |
