...
- `src/projects/extended_gaussian/*` 계층을 중심으로 기능을 추가한다.
- 초기 목표는 **viewer 기능**이며, 편집기 전체를 완성하는 것이 아니다.
- 구현은 **GPU-only swap**을 먼저 완성한 뒤, 이후 **CPU unload**, 마지막으로 필요 시 **chunk 단위 streaming**으로 확장한다.

현재 코드베이스는 SIBR의 윈도우, 멀티뷰, 카메라, 렌더 타깃 프레임워크를 재사용하고, `extended_gaussian` 프로젝트 계층에서 Gaussian 자산 로딩, 씬 인스턴스, GPU 자원 관리, world buffer 재조립, CUDA rasterizer 호출을 담당하고 있다. 따라서 본 설계는 **SIBR 전체 개조가 아니라 extended_gaussian 계층에 streaming / residency 기능을 삽입하는 방향**을 전제로 한다.
...
```text
src/projects/extended_gaussian/renderer/subsystem/streaming_system/
  StreamingSystem.hpp
  StreamingSystem.cpp
  ManifestStore.hpp
  ManifestStore.cpp
...
1. **SIBR core는 유지하고 `extended_gaussian` 계층만 수정한다.**
...
**본 설계는 SIBR 프레임워크 위의 `extended_gaussian` 계층에 `manifest + asset_id + residency manager + swap policy + background loader`를 추가하여, 현재 상태에 따라 Gaussian 자산을 안전하게 자동 스왑하는 viewer를 단계적으로 구현하는 계획이다.**
